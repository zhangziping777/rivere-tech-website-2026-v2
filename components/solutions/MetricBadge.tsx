...
export default function MetricBadge({ value, unit, label }: MetricBadgeProps) {
  return (
    <div className="group relative flex flex-col items-center px-6 py-5 rounded-sm border border-brand-border bg-brand-surface/40 hover:border-cyan-500/40 hover:-translate-y-0.5 transition-all duration-500 ease-out">
      {/* glow on hover */}
      ...
      <div className="relative z-10 flex items-baseline gap-0.5">
        <span className="text-4xl md:text-5xl font-bold text-cyan-500"
          style={{ textShadow: "0 0 32px rgba(6,182,212,0.25), 0 0 8px rgba(6,182,212,0.15)" }}>
          <CountUpMetric value={value} />
        </span>
        <span className="text-lg md:text-xl font-semibold text-cyan-500/70">
          {unit}
        </span>
      </div>
